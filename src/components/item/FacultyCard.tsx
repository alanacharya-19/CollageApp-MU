      ...
      <NeumorphicBox radius={neu.radius.xl}>
        <View className="p-4">
          <View className="flex-row items-center">
            <View className="w-14 h-14 rounded-full items-center justify-center" style={{ backgroundColor: item.color + "18" }}>
              <Text className="text-lg font-extrabold" style={{ color: item.color }}>
                {item.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
              </Text>
            </View>
            <View className="flex-1 ml-3">
              <Text className="text-sm font-bold" style={{ color: neu.text }}>{item.name}</Text>
              <Text className="text-xs font-semibold mt-0.5" style={{ color: item.color }}>{item.designation}</Text>
