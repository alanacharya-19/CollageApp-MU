...
export default function ProgramCard({ item }: { item: Program }) {
  return (
    <Pressable className="active:opacity-80 mb-3">
      <NeumorphicBox radius={neu.radius.xl}>
        <View className="p-4">
          <View className="flex-row items-center">
            <View className="w-12 h-12 rounded-2xl items-center justify-center" style={{ backgroundColor: neu.dark + "20" }}>
              <Ionicons name={item.icon} size={22} color={neu.textMuted} />
            </View>
            <View className="flex-1 ml-3">
              <View className="flex-row items-center">
                <Text className="text-[11px] font-bold" style={{ color: neu.accent }}>{item.degree}</Text>
          ...
          <Text className="text-xs leading-4 mt-3" style={{ color: neu.textMuted }} numberOfLines={2}>
            {item.description}
          </Text>
          ...
          <View className="flex-row items-center mt-3 pt-3" style={{ borderTopWidth: 1, borderTopColor: neu.dark + "25" }}>
            ...
            <View className="mx-3 w-px h-3" style={{ backgroundColor: neu.dark + "30" }} />
            <View className="flex-row items-center">
